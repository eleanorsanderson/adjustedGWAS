...
dat <- datagen("correlated",250,0.4,0.8) 
...
bmi_out <- summary(lm(out_badj~-1+bmi_b,weights=1/(out_se^2), data = MR_dat.bmi))
sbp_out <- summary(lm(out_badj~-1+sbp_b,weights=1/(out_se^2), data = MR_dat.sbp))
sbpaj_out <- summary(lm(out_badj~-1+sbpaj_b,weights=1/(out_se^2), data = MR_dat.sbpaj))
mvmr_out <- summary(lm(out_badj~-1+bmi_b+sbp_b,weights=1/(out_se^2), data = MR_dat.mvmr))
mvmraj_out <- summary(lm(out_badj~-1+bmi_b+sbpaj_b,weights=1/(out_se^2), data = MR_dat.mvmraj))
...
write.csv(results, file = "/mnt/storage/scratch/bu19525/BristolPhD/MVMRproject/output/pos/outcome/results_corr_pos_onlyout_5.csv")
